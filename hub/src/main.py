...
from app.adapters.store_api_adapters import StoreApiAdapter
from app.entities.agent_data import AgentData
from app.entities.processed_agent_data import ProcessedAgentData, process_agent_data
from config import STORE_API_BASE_URL, REDIS_HOST, REDIS_PORT, BATCH_SIZE, MQTT_BROKER_HOST, MQTT_BROKER_PORT, \
    MQTT_TOPIC
...
def on_message(client, userdata, msg):
    try:
        logging.error(f"Received message: {msg.topic} {msg.payload}")
        payload: str = msg.payload.decode("utf-8")
        # Create ProcessedAgentData instance with the received data
        agent_data = AgentData.model_validate_json(payload, strict=True)
        processed_agent_data = process_agent_data(agent_data)
        redis_client.lpush(
            "processed_agent_data",
            processed_agent_data.model_dump_json()
        )
